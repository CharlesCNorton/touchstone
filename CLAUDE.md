...
| `vcgen.py` | the Rocq-extracted weakest-precondition generator (`wpg`/`vcg`), driven through `_generated/vcgen_rocq.py`, with a CPython cross-check of the (unproven) Python→IR lowering. |
...
| `diagnostics.py` / `repro.py` / `certificate.py` | UNKNOWN classification + the `covers` reference; REFUTED→runnable failing test; re-checkable proof bundles. |
...
- a verb or engine → `engines.py`; if it is a trap-freedom strategy, also wire it into `benchmark._decide`, add a `verification_benchmark` case, and a `run_self_tests` assert.
