...
def transpile(json_path):
    """The Python image of the Rocq JSON extraction at json_path -- byte-for-byte the text the command-line
    form writes -- so a stock install can hold each committed _generated/*.py equal to it with no Coq toolchain."""
    with open(json_path, encoding="utf-8") as f:
        return Translator().module(json.load(f))
